...
if not os.path.exists(env_path):
    logging.error(f"Файл .env не найден по пути: {env_path}")
...
logging.info(f"DISCORD_TOKEN установлен: {'Да' if DISCORD_TOKEN else 'Нет'}")
if DISCORD_TOKEN:
    logging.info(f"Длина DISCORD_TOKEN: {len(DISCORD_TOKEN)} символов")
    logging.info(f"Первые 5 символов токена: {DISCORD_TOKEN[:5]}...")
else:
    logging.error("DISCORD_TOKEN не найден в переменных окружения")
...
class PartyView(View):
    def __init__(self, party_text, creator):
        super().__init__(timeout=None)
        self.party_text = party_text
        self.creator = creator
        self.positions = []
        self.message = None  # Добавляем атрибут для хранения сообщения
        self.log_channel = creator.guild.get_channel(LOG_ALL_CHANNEL_ID)
        self.parse_positions()
        self.update_buttons()

    def create_embed(self):
        """Создает и возвращает объект discord.Embed для отображения сбора"""
        embed = discord.Embed(
            title="Сбор участников",
            description=self.party_text,
            color=discord.Color.blue()
        )
        embed.set_author(name=self.creator.display_name, icon_url=self.creator.avatar.url if self.creator.avatar else None)
        return embed

    def parse_positions(self):
        lines = self.party_text.split('\n')
        for line in lines:
            line = line.strip()
            if line.endswith('-'):
                parts = line[:-1].strip().split('.')
                if len(parts) >= 1 and parts[0].isdigit():
                    position_number = int(parts[0])
    ...
    def update_buttons(self):
        ...
        # Кнопка "Записаться"
        join_button = Button(label="Записаться", style=discord.ButtonStyle.success, custom_id="join")
        join_button.callback = self.join_button_callback
        self.add_item(join_button)

        # Кнопка "Выписаться"
        leave_button = Button(label="Выписаться", style=discord.ButtonStyle.danger, custom_id="leave")
        leave_button.callback = self.leave_button_callback
        self.add_item(leave_button)

        # Кнопка "Управление участниками"
        manage_button = Button(label="Управление участниками", style=discord.ButtonStyle.secondary, custom_id="manage")
        manage_button.callback = self.manage_button_callback
        self.add_item(manage_button)

    async def join_button_callback(self, interaction: discord.Interaction):
        # Логика для обработки нажатия кнопки "Записаться"
        if self.log_channel:
            await self.log_channel.send("🔍 **DEBUG:** Вызван join_button_callback")
        try:
            # Передаем сообщение в модальное окно
            modal = JoinModal(self.positions, interaction.message, self.log_channel)
            await interaction.response.send_modal(modal)
            if self.log_channel:
                await self.log_channel.send("🔍 **DEBUG:** Модальное окно JoinModal успешно отправлено")
        except Exception as e:
            if self.log_channel:
                await self.log_channel.send(f"❌ Ошибка при отправке модального окна: {str(e)}")

    async def leave_button_callback(self, interaction: discord.Interaction):
        # Логика для обработки нажатия кнопки "Выписаться"
        await interaction.response.send_modal(LeaveModal(self.positions))

    async def manage_button_callback(self, interaction: discord.Interaction):
        try:
            if not await has_role(interaction.user, CONTENT_MAKER_ROLE_ID) and interaction.user != self.creator:
                await interaction.response.send_message("❌ У вас нет прав для управления участниками.", ephemeral=True)
                return

            # Сохраняем сообщение, если оно еще не сохранено
            if not self.message:
                self.message = interaction.message

            view = ManagePartyView(self)
            await interaction.response.send_message("🔧 Управление участниками:", view=view, ephemeral=True)

            if self.log_channel:
                await self.log_channel.send(f"🔍 **DEBUG:** Открыто управление участниками пользователем {interaction.user.mention}")

        except Exception as e:
            if self.log_channel:
                await self.log_channel.send(f"❌ **DEBUG:** Ошибка в manage_button_callback: {str(e)}")
            await interaction.response.send_message("❌ Произошла ошибка при открытии управления участниками", ephemeral=True)

class JoinModal(discord.ui.Modal):
    def __init__(self, positions, message, log_channel=None):  # Добавляем параметр message
        super().__init__(title="Записаться на позицию")
        self.positions = positions
        self.message = message  # Сохраняем сообщение
        self.log_channel = log_channel
        self.add_item(discord.ui.TextInput(
            label="Введите номер позиции",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            # Отладка начала обработки
            print(f"DEBUG: Начало обработки формы. User: {interaction.user}")
            if self.log_channel:
                await self.log_channel.send(f"🔍 **DEBUG:** Начало обработки формы от {interaction.user}")
            
            # Получаем введенное значение
            position_input = self.children[0].value
            print(f"DEBUG: Введено значение: {position_input}")
            
            # Проверяем, является ли введенное значение числом
            if not position_input.isdigit():
                raise ValueError("Введите числовое значение")
            
            position_number = int(position_input)
            print(f"DEBUG: Преобразовано в число: {position_number}")
            
            # Проверяем диапазон
            if position_number < 1 or position_number > len(self.positions):
                print(f"DEBUG: Номер позиции вне диапазона: {position_number}, доступно: 1-{len(self.positions)}")
                raise ValueError(f"Номер позиции должен быть от 1 до {len(self.positions)}")
            
            # Проверяем, не занята ли позиция
            if self.positions[position_number - 1]['member'] is not None:
                print(f"DEBUG: Позиция {position_number} уже занята")
                raise ValueError("Эта позиция уже занята")
            
            # Записываем пользователя на позицию
            self.positions[position_number - 1]['member'] = interaction.user
            print(f"DEBUG: Пользователь {interaction.user} записан на позицию {position_number}")
            
            # Обновляем основное сообщение
            lines = self.message.embeds[0].description.split('\n')
            for i, line in enumerate(lines):
                if line.strip().startswith(f"{position_number}."):
                    lines[i] = f"{line.split('-')[0]}- {interaction.user.mention}"
            ...
            await interaction.response.send_message(
                f"✅ Вы успешно записались на позицию {position_number}",
                ephemeral=True
            )
            
            if self.log_channel:
                await self.log_channel.send(
                    f"🔍 **DEBUG:** Пользователь {interaction.user.mention} успешно записан на позицию {position_number}"
                )
            
        except ValueError as e:
            print(f"DEBUG: Ошибка ValueError: {str(e)}")
            await interaction.response.send_message(f"❌ Ошибка: {str(e)}", ephemeral=True)
            if self.log_channel:
                await self.log_channel.send(f"❌ **DEBUG:** Ошибка ValueError для {interaction.user.mention}: {str(e)}")
            ...
            if self.log_channel:
                await self.log_channel.send(f"❌ **DEBUG:** Непредвиденная ошибка для {interaction.user.mention}: {str(e)}")

    async def on_error(self, interaction: discord.Interaction, error: Exception) -> None:
        print(f"DEBUG: Ошибка в модальном окне: {str(error)}")
        await interaction.response.send_message("❌ Произошла ошибка при обработке формы", ephemeral=True)
        if self.log_channel:
            await self.log_channel.send(f"❌ **DEBUG:** Ошибка в модальном окне для {interaction.user.mention}: {str(error)}")

class LeaveModal(discord.ui.Modal):
    def __init__(self, positions):
        super().__init__(title="Выписаться с позиции")
        self.positions = positions
        self.add_item(discord.ui.TextInput(label="Введите номер позиции"))

    async def callback(self, interaction: discord.Interaction):
        position_number = int(self.children[0].value)
        # Логика для выписки с позиции
        await interaction.response.send_message(f"Вы выписаны с позиции {position_number}", ephemeral=True)

@bot.command(name="party")
async def party(ctx):
    """Базовая команда для работы со сборами"""
    await ctx.send("Используйте `!party create [описание]` для создания сбора")

@bot.command(name="party_create", aliases=["party create"])
async def party_create(ctx, *, content=""):
    """Создает новый сбор с кнопками управления"""
    log_channel = ctx.guild.get_channel(LOG_ALL_CHANNEL_ID)

    async def log_debug(message):
        """Отправляет отладочное сообщение в канал логов"""
        if log_channel:
            await log_channel.send(f"🔍 **DEBUG:** {message}")

    await log_debug(f"Полученный текст: {content}")

    # Проверяем, есть ли текст
    if not content:
        await ctx.send("❌ Укажите описание сбора и позиции. Пример:\n```\n!party create Сбор на рейд\n1. -\n2. -\n3. -\n```")
        await log_debug("❌ Описание сбора не указано.")
        return

    # Проверяем права на создание сбора
    if not await has_role(ctx.author, CONTENT_MAKER_ROLE_ID):
        await ctx.send("❌ У вас нет прав для создания сбора")
        await log_debug("❌ У пользователя нет прав для создания сбора.")
        return

    # Создаем View с кнопками
    view = PartyView(content, ctx.author)
    await log_debug(f"Найденные позиции: {view.positions}")

    # Если в сообщении нет позиций с дефисами
    if not view.positions:
        await ctx.send("❌ В сообщении нет позиций для записи. Укажите позиции в формате:\n```\n1. -\n2. -\n```")
        await log_debug("❌ В сообщении нет позиций для записи.")
        return

    # Отправляем сообщение с кнопками
    embed = view.create_embed()
    message = await ctx.send(embed=embed, view=view)
    view.message = message  # Сохраняем сообщение в view
    await log_debug(f"Сообщение отправлено: {message.id}")

    # Удаляем команду создания
    try:
        await ctx.message.delete()
        await log_debug("Команда создания успешно удалена.")
    except Exception as e:
        await log_debug(f"Ошибка при удалении сообщения: {e}")
...
async def help_command(ctx):
    help_message = (
        ...
        "`!telegram_panel` - Создать панель управления Telegram с кнопками (только администратор).\n\n"
        "**💰 Управление балансом:**\n"
        "`!balance` - Показать свой текущий баланс.\n"
        ...
        "`!balance_update` - Массовое обновление балансов (только администратор).\n"
        "`!reset_all_balances` - Обнулить все балансы (только администратор).\n\n"
        "**🚫 Управление штрафами:**\n"
        ...
        "`!link_telegram` - Привязать аккаунт Telegram для получения уведомлений.\n"
        "`!get_user_id [ник]` - Получить ID пользователя по нику (только администратор).\n\n"
...
async def main():
    try:
        ...
        if not DISCORD_TOKEN:
            logging.error("DISCORD_TOKEN не установлен")
            raise ValueError("DISCORD_TOKEN не установлен")
            
        logging.info(f"Запуск бота с токеном длиной {len(DISCORD_TOKEN)} символов")
        ...
        try:
            ...
            logging.error(f"Ошибка авторизации Discord: {e}")
            raise
        except Exception as e:
            logging.error(f"Ошибка при запуске Discord бота: {e}")
            raise
            
        # Ждем завершения Telegram бота
        await telegram_task
...
class ManagePartyView(discord.ui.View):
    ...
    async def remove_member_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not await has_role(interaction.user, CONTENT_MAKER_ROLE_ID) and interaction.user != self.party_view.creator:
            await interaction.response.send_message("❌ У вас нет прав для управления участниками.", ephemeral=True)
            return
        
        await interaction.response.send_modal(RemovePartyMemberModal(self.party_view))

    @discord.ui.button(label="Добавить участника", style=discord.ButtonStyle.success)
    async def add_member_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not await has_role(interaction.user, CONTENT_MAKER_ROLE_ID) and interaction.user != self.party_view.creator:
            await interaction.response.send_message("❌ У вас нет прав для управления участниками.", ephemeral=True)
            return
        
        await interaction.response.send_modal(AddPartyMemberModal(self.party_view))

class RemovePartyMemberModal(discord.ui.Modal):
    def __init__(self, party_view):
        super().__init__(title="Удалить участника")
        self.party_view = party_view
        self.add_item(discord.ui.TextInput(
            label="Номер позиции",
            placeholder="Например: 1",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if position_number < 1 or position_number > len(self.party_view.positions):
                raise ValueError(f"Номер позиции должен быть от 1 до {len(self.party_view.positions)}")
            
            # Обновляем основное сообщение
            lines = self.party_view.message.embeds[0].description.split('\n')
            for i, line in enumerate(lines):
                if line.strip().startswith(f"{position_number}."):
                    lines[i] = f"{position_number}. {line.split('-')[0].split('.')[1].strip()} -"
                    self.party_view.positions[position_number - 1]['member'] = None
                    break
            
            updated_embed = self.party_view.message.embeds[0]
            updated_embed.description = '\n'.join(lines)
            await self.party_view.message.edit(embed=updated_embed)
            
            await interaction.response.send_message("✅ Участник удален с позиции.", ephemeral=True)
...
class AddPartyMemberModal(discord.ui.Modal):
    def __init__(self, party_view):
        ...
        self.add_item(discord.ui.TextInput(
            ...
            placeholder="Например: 123456789 или @пользователь",
            min_length=1,
            required=True
        ))
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            position_number = int(self.children[0].value)
            member_input = self.children[1].value.strip()
            ...
            if member_input.isdigit():
                ...
                mention_match = re.match(r"<@!?(\d+)>", member_input)
                if mention_match:
                    member_id = int(mention_match.group(1))
            ...
            if self.party_view.positions[position_number - 1]['member'] is not None:
                raise ValueError("Эта позиция уже занята")
            ...
            for i, line in enumerate(lines):
                if line.strip().startswith(f"{position_number}."):
                    lines[i] = f"{line.split('-')[0]}- {member.mention}"
                    self.party_view.positions[position_number - 1]['member'] = member
                    break
            ...
            await interaction.response.send_message(f"✅ {member.mention} добавлен на позицию {position_number}.", ephemeral=True)
